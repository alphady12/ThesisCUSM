...
const Reservations = () => {
  ...
  const handleRoomClick = (room, floorNumber) => {
    ...
    if (isReserved || room.status === 'Reserved'|| room.status === 'Disabled') {
     ...
     if(isPending || room.status === 'Pending' || room.status === 'Disabled') {
  return;
  ...
  {buildings.length > 0 ? (
    <>
      ...
      {selectedBuilding && (
        <>
          {buildings.map((building) => {
            if (building.bldg_name === selectedBuilding) {
              return (
                <div key={building.bldg_id}>
                  <div className="buildingavails-details">
                    <Card style={{ width: "300px", padding: "10px", borderRadius: "10px", boxShadow: "0 2px 5px rgba(0,0,0,0.2)", marginTop: '-60px', marginLeft: '-65px' }}>
                      <Card.Body>
                  ...
                  <div style={{ marginBottom: '10px', display: 'flex', alignItems: 'center', marginLeft: '455px', marginTop: '-50px' }}>
                  ...
                  <div className="customer-container" style={{ width: '140%', marginLeft: '-180px' }}>
                    <Table striped bordered style={{ width: '1955px', minWidth: '300px', marginLeft: '-299px', height: '90px', marginTop: '-30px' }}>
                      <thead>
                        <tr>
  ...
  {floor.rooms.length > 0 ? (
      ...
      .map((room) => {
        ...
        return (
          ...
          >
            ...
            return null; // Return null for buildings that don't match the selected building
          })}
        </>
      )}
    </>
    ...
    <p>No buildings available.</p>
      ...
      <Modal show={showReservationModal} onHide={handleCloseReservationModal} centered dialogClassName="custom-modal">
